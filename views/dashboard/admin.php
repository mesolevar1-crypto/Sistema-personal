...
$usuarios = array_slice($usuarios, ($paginaActual - 1) * $porPagina, $porPagina);
...
.pag{width:36px;height:36px;border:1px solid #E5E7EB;border-radius:9px;background:white;color:#5F6673;display:flex;align-items:center;justify-content:center;text-decoration:none;font-weight:600}
.pag:hover{background:#DDF5EC;border-color:#61D0A7;color:#01614B}
.pag.activa{background:#00875F;border-color:#00875F;color:white}
        ...
        <div class="overflow-x-auto">
            ...
            <table class="w-full text-left border-collapse">
                ...
                <?php foreach ($usuarios as $u): ?>
                    ...
                    <?php
                    $estado = $u['estado_usuario'] ?? ($u['estado'] ?? 1);
                    $activo = $estado === 1 || $estado === '1' || strtolower((string)$estado) === 'activo';
                    $fila = $activo ? '#FFFFFF' : '#FDECEC';
                    $hover = $activo ? '#F8F8F8' : '#FDE0E0';
                    $id = (int)($u['id_usuario'] ?? 0);
                    $admin = strtolower($u['nombre_rol'] ?? '') === 'administrador';
                    ?>
                    ...
                    >
                        ...
                        <td class="px-5 py-3.5 text-sm" style="color:#5F6673">
                            <?= htmlspecialchars($u['telefono'] ?: 'No registrado', ENT_QUOTES, 'UTF-8') ?>
                        </td>
                            ...
                            <div style="display:flex;justify-content:center;align-items:center;gap:8px">
                                ...
                                <button
                                    ...
                                    onclick='abrirModalEliminar(
                                        <?= $id ?>,
                                        <?= json_encode($u['nombre']??'',JSON_HEX_TAG|JSON_HEX_APOS|JSON_HEX_QUOT|JSON_HEX_AMP) ?>
                                    )'
...
function confirmarEliminar(){
    ...
    location.href =
        '../../controllers/AdminUsuarioController.php?accion=eliminar&id=' +
...
function cambiarEstadoUsuario(id){
    ...
    location.href =
        '../../controllers/AdminUsuarioController.php?accion=toggleEstado&id=' +
